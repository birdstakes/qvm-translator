...
def accept_lists_as_addresses(f):
    def wrap(*args):
        args = [
            EffectiveAddress.from_list(arg) if isinstance(arg, list) else arg
            for arg in args
        ]
        f(*args)
...
class Assembler:
        ...
        for label in self.labels:
            ...
            for use in label.uses:
                struct.pack_into(
                    "<I",
                    self.code,
                    use.address - self.base,
                    (label.address - use.relative_to) & 0xFFFFFFFF,
                )
    ...
    def call(self, target):
        if isinstance(target, Label):
            self.emit([0xE8])
            target.uses.append(
                LabelUse(self.current_address(), relative_to=self.current_address() + 4)
            )
            self.emit32(0)
